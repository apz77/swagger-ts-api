...
import { getErorrProperty } from '../schemaProcessor/schema';
...
export class ObjectTypeProcessor implements TypeProcessor {
  ...
  consume(swaggerSchemaProperty: SwaggerSchemaProperty,
    ...
    if (typeName === 'object') {
      if (swaggerSchemaProperty.properties &&
        Object(swaggerSchemaProperty.properties) === swaggerSchemaProperty.properties) {

        const properties: {[key: string]: ObjectProperty} = {};

        for (const propertyName in swaggerSchemaProperty.properties) {
