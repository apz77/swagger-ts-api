...
export interface PropertyProcessor {
    consume: (name: string, property: SwaggerSchemaProperty, isRequired: boolean, ctx: PropertyProcessorContext) => ObjectProperty | null
}
...
export class SchemaPropertyFactory {
    ...
    translateProperty(schemaName: string, swaggerProperty: SwaggerSchemaProperty, isRequired: boolean, ctx: PropertyProcessorContext) {
        for(let processor of this.propertyProcessors) {
